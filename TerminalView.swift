...
struct TerminalView: View {
    ...
    private func stripANSIEscapeCodes(
        _ text: String
    ) -> String {

        let pattern =
            "\u{1B}(\\[[0-9;?]*[a-zA-Z]|\\][^\u{07}]*\u{07}|[()][A-Za-z0-9]|[@-Z\\\\^_])"
    ...
    private func renderOutputLines(
        ...
        LazyVStack(
            ...
            if isTruncated {
                ...
                // ★ 修复：
                // 原来的 "复制整段" 会把 Swift 字符串提前结束。
                // 这里使用中文引号，避免转义问题。

                Text(
                    "⚠️ 输出过长（共 \(allLines.count) 行），仅显示最后 \(maxRenderedLinesPerBlock) 行。点击“复制整段”可获取完整内容。"
                )
                .font(
                    .system(
                        size: 11,
    ...
    private func copyValueForOutputLine(
        _ line: String
    ) -> String? {
